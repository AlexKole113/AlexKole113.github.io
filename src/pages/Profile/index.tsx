...
const Profile = () => {
  ...
  const getProfileGroups = (fields:IFakeProfileFields, activeID:number) => {
    const groups = [];
    for (const groupName in fields) groups.push(<ProfileGroup key={groupName} name={groupName} fields={fields[groupName].fields} active={(activeID === fields[groupName].id) ? 'active' : ''} pageState={state} updatePageState={setState} />);
    return groups;
  };
